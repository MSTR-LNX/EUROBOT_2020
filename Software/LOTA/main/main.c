        ...
        switch(msg[0])
        {
          case 'L':
            if(sscanf(msg, "L%i", &value)) {
              ESP_LOGI(TAG, "Received value: %i", value);
...
// handles clients when they first connect and pass each client to queue
...
// receive clients from queue, handle them
...
static void count_task(void* pvParameters)
{
  ...
  uint32_t n = 0;
  const int DELAY_MS = 10 / portTICK_PERIOD_MS;
